       ...
       this.recaptchaV3Service.execute('forgotPassFormSubmission').subscribe({
         next: (token) => {
          const data: any = {
            ...
            template: '/modules/hub/magic.startup/templates/reset-password.html',
          }

          this.authApiService.forgotPass(data).subscribe({
            next: (res: any) => {
              this.waiting = false;
            ...
            error: (error: any) => {
              this.generalService.showFeedback(error.error?.message || error);
              this.waiting = false;
            }});
        },
        error: (e) => {
